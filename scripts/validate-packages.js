...
async function validatePackage(packageDirectory, manifest) {
	...
	try {
		...
		await writeFile(join(temporaryRoot, 'import.mjs'), `import * as packageModule from ${JSON.stringify(packageName)}\nvoid packageModule\n`)
		...
		if (supportsRequire) {
			await writeFile(join(temporaryRoot, 'require.cjs'), `const packageModule = require(${JSON.stringify(packageName)})\nvoid packageModule\n`)
...
async function validateTypeScriptConsumers(temporaryRoot, packageName, supportsRequire) {
	await writeFile(join(temporaryRoot, 'consumer.ts'), `import * as packageModule from ${JSON.stringify(packageName)}\nvoid packageModule\n`)
	...
	if (supportsRequire) {
		await writeFile(join(temporaryRoot, 'consumer.cts'), `import packageModule = require(${JSON.stringify(packageName)})\nvoid packageModule\n`)
...
function hasCondition(value, condition) {
	...
	return Object.entries(value)
		.some(([key, nestedValue]) => (key === condition || hasCondition(nestedValue, condition)))
}
async function writeJson(filePath, value) {
	await mkdir(dirname(filePath), { recursive: true })
	await writeFile(filePath, `${JSON.stringify(value, null, '\t')}\n`)
}
async function runTypeScript(projectPath) {
